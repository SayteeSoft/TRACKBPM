...
export default function Home() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    try {
      // Basic parsing of "artist - title" format
      ...
      setResults((prevResults) => [{ ...result, artist, title }, ...prevResults]);
      form.reset();
    } catch (error) {
      toast({
        variant: "destructive",
        ...
        {results.length > 0 && (
          <section className="mt-12 space-y-4 max-w-3xl mx-auto">
            {results.map((result, index) => (
              <Card key={`${result.title}-${index}`} className="overflow-hidden shadow-md animate-in fade-in-0 duration-500">
                <div className="flex">
